/*
 * spcat-obj.c - SPCAT as a linkable object
 *
 * Based on original calcat.c
 */
/***********************************************************************/
...
const char *spcat_ext[NFILE] = { "int", "var", "out", "cat", "str", "egy" };

/************** CALCAT interfaces ***********************************/
int qnfmt(short *iqu, int nqn, /*@out@*/char *sqn);
int simt(int isiz, int jsiz, double *s, double *t, double *u,
                double *wk);
/*@dependent@*/ SBLK *ibufof(spcs_t *s, const int iblk,
...
FILE *fopenbr(char **filebuf, size_t *bufsize)
{
  FILE *hret;
  hret = fmemopen(*filebuf, *bufsize, "r");
  if(hret == NULL) {
    perror("spcat: Trouble opening memory buffer for read");
    exit(1);
  }
  return hret;
} /* fopenq */
FILE *fopenbw(char **filebuf, size_t *bufsize)
{
  FILE *hret;
  hret = open_memstream(filebuf, bufsize);
  if(hret == NULL) {
    perror("spcat: Trouble opening memory buffer for write");
    exit(1);
  }
  return hret;
} /* fopenq */

int init_spcs(spcs_t *spcs) {
  ...
  */  
  ...
  /*             ---  ---  ---- ---  ---                          */
  /*              0    1    2    3    4                           */
  static const short sptzero[2] = { 1, 0 };
  static const SSP ssp_head = { NULL, NULL/*sptzero*/, 1 , 0};
  static const int ixphase[4] = {0, 1, 2, 3};
  static const char csym[4] = { 'x', 'c', 'b', 'a' };
  memset(spcs, 0, sizeof(spcs_t)); /* Initialize to zero */
  spcs->itpairs = malloc(sizeof(itpairs));
  if ( !spcs->itpairs ) return 1;
  memcpy(spcs->itpairs, itpairs, sizeof(itpairs));
  spcs->itpair = spcs->itpairs;
  spcs->vinfo = &spcs->vinfo1;
  memcpy(spcs->sptzero, sptzero, sizeof(sptzero));
  memcpy(&spcs->ssp_head, &ssp_head, sizeof(SSP));
  spcs->ssp_head.sspt = spcs->sptzero;
  spcs->dipinfo = &spcs->dipinfo0;
  memcpy(spcs->ixphase, ixphase, sizeof(ixphase));
  spcs->nold = -1;
  memcpy(spcs->csym, csym, sizeof(csym));
  spcs->oldgsym = -1;
  spcs->nlast = -1;
  spcs->klast = 0;
  return 0;
}
int free_spcs(spcs_t *spcs) {
  int i;
  /* Maybe correct. Not sure if freed or closed but not null'd. */
  if ( spcs->scratch ) fclose(spcs->scratch); /* Never closed or nulled */
  if ( spcs->itpairs ) free(spcs->itpairs); /* Never freed or nulled */
  /* spcs->ipder sometimes freed then either nulled or made ptr to zipder */
  if ( spcs->ipder && spcs->ipder != &spcs->zipder ) free(spcs->ipder);
  /* spcs->dipinfo sometimes mallocd, sometimes freed and nulled.
   * Defaults to ptr to dipinfo0 */
  if ( spcs->dipinfo && spcs->dipinfo != &spcs->dipinfo0 ) free(spcs->dipinfo);
  /* Maybe correct. Not sure how this list works */
  for (i = 0; i < spcs->glob.parinit; ++i) {
    while ( spcs->spar_head[i] ) {
      PSPAR temp = spcs->spar_head[i]->next;
      if ( spcs->spar_head[i] ) free(spcs->spar_head[i]);
      spcs->spar_head[i] = temp;
    }
  }
  return 0;
}
...
int spcat(spcs_t *spcs, char *filebufs[], size_t bufsizes[]) /* Must have NFILE of them */
{
#define NTEMP 8
#define NCARD 130
#define NDVEC 10
 /* Initialized data */
  ...
  /*static*/const char warn[] = " WARNING: THERE WAS NO DIAGONALIZATION\n";
  /*static*/const char headq[] = "TEMPERATURE - Q(SPIN-ROT.) - log Q(SPIN-ROT.)\n";
  /* Local variables */
  ...
  luout = fopenbw(&filebufs[eout], &bufsizes[eout]);
  luegy = lustr = luout;
  luint = fopenbr(&filebufs[eint], &bufsizes[eint]);
  nline = 0;
  egymin = 0.;
  ...
  iflg %= 100;
  if (iflg >= 10) {
    lustr = fopenbw(&filebufs[estr], &bufsizes[estr]);
    prstr = TRUE;
  }
  stcomp = 1. / zero;
  npdip = 1;
  ...
  preig = (iflg >= 3);
  if (iflg != 0) {
    luegy = fopenbw(&filebufs[eegy], &bufsizes[eegy]);
    pregy = TRUE;
  }
  ...
  luvar = fopenbr(&filebufs[evar], &bufsizes[evar]);
  /* read var file */
  npar = 0; catqn = MAXCAT;
  ...
  /* read option line(s) */
  nfmt = catqn;
  if (npar > 0 && setopt(spcs, luvar, &nfmt, &itd, &ndbcd, titl) <= 0)
    npar = 0;
  if (npar <= 0) {
  ...
  iqnfmtv[0] = 0;
  nqn = setfmt(spcs, iqnfmtv, nfmt);
  memset(sqn, 0, sizeof(sqn));	/* Not initialized properly */
  if (nqn > MAXCAT) {
    catqn = nqn;
  } else {
    sqn[24] = ' ';
  ...
  setblk(spcs, luout, npar, idpar, par, &nbkpj, &i);
  maxdm = (unsigned) i;
  inblk = nbkpj * inblk + 1;
  lblk = nbkpj * (lblk + 1);
  ...
  /* set up DIAG,NSAV */
  setint(spcs, luout, &diag, &nsav, ndip, idip, isimag);
  if (prstr) {
    k = 0; i = nvdip[0]; ij = 0;
  ...
  strmn = pow(10., strmn);
  maxqn = getqn(spcs, inblk, 0, 0, iqni, &isiz);
  if (maxqn > MAXQNX)
    maxqn = MAXQNX;
  ktsp = -1;
  lucat = fopenbw(&filebufs[ecat], &bufsizes[ecat]);
  /**********************************************************************/
  ...
  for (iblk = inblk; iblk <= lblk; ++iblk) {
    ...
    if ((iblk - j * nbkpj) == 1) {
      ...
      /* -- Removed to avoid static variables
    ...
    pblk = ibufof(spcs, -1, ndel, blk);
    /* get energy and derivatives for new block */
    getqn(spcs, iblk, 0, 0, iqni, &isiz);
    if (isiz <= 0)
      continue;
    ...
    if (egy == NULL)
      break;
    dedp = egy + isiz;
    hamx(spcs, iblk, isiz, npar, idpar, par, egy, teig, dedp, pmix, ifdump);
    ...
    for (i = 0; i < isiz; ++i, teigp += isiz) {
      if (ifdump)
        kbgn = i;
      getqn(spcs, iblk, i + 1, nqn, iqni, &idgn);
      if (idgn <= 0) 
        continue;
      iv = iqni[iposv];
      if (iv > maxv)
      ...
      if (globfmt != 0) {
        ...
        if (newfmt != 0) 
          ktsp = iqni[nqn - 2];
        getqn(spcs, iblk, i + 1, maxqn, iqni, &idgn);
      }
    ...
    do {                        /* loop over previous blocks for intensity */
      ...
      for (i = 0; i < npdip; ++i) {     /*  get intensity */
        sij = s[i];
        idf = intens(spcs, iblk, isiz, jblk, jsiz, nvdip[i],
      ...
      for (i = 0; i < isiz; ++i) {
        ...
        for (j = 0; j < jmax; ++j) {
          if (i - j + idf < 0)
            break;
          getqn(spcs, jblk, j + 1, nqn, &iqni[MAXQN], &jdgn);
  ...
  if (prstr)
    fclose(lustr);
  setblk(spcs, luout, 0, idpar, par, &nbkpj, &i);     /* release storage */
  fclose(luout);
  fclose(lucat);
  /* free up memory */
  pblk = blk;
  ...
  free(dip);
  free(dvec);
  /* sort cat file -- skipping for now!!! */
  /* sortn(fname[ecat], fname[ecat], FALSE); */
  //sortbn(&filebufs[ecat], &bufsizes[ecat], &filebufs[ecat], &bufsizes[ecat], FALSE);
  return 0;
}                               /* main */
...
SBLK *ibufof(spcs, ipos, ndel, blk)
spcs_t *spcs;
const int ipos;
const unsigned int ndel;
SBLK *blk;
{
  ...
  if (pmblk->egyblk == NULL) {  /* write memory contents to disk */
    if (spcs->maxrec == 0) {
      spcs->scratch = tmpfile();
      if (spcs->scratch == NULL) {
        puts("scratch file open error");
        exit(EXIT_FAILURE);
      }
      spcs->nbsav = spcs->mempos;
      spcs->maxdm = pmblk->nsizblk;
      ldisk = (long) ndel;
      if (blk->eigblk != NULL)
        ldisk += (long) spcs->maxdm;
      spcs->lsizb = (long) spcs->maxdm *ldisk * (int) sizeof(double);
    }
    ldisk = (long) (spcs->mempos - spcs->nbsav);
    if (ldisk != 0)
      ldisk *= spcs->lsizb;
    fseek(spcs->scratch, ldisk, SEEK_SET);
    spcs->mempos = k + 1;
    if (spcs->mempos == spcs->orgpos)
      ++spcs->mempos;
    if (spcs->mempos >= spcs->nbsav)
      spcs->mempos = (spcs->orgpos != 0) ? 0 : 1;
    pdblk = pmblk;
    pmblk = &blk[spcs->mempos];
    len = (long) ndel;
    ...
    lret = 0;
    if (pvec != NULL) {
      lret = (long) fwrite(pvec, sizeof(double), (size_t) len, spcs->scratch);
      if (lret == len && pmblk->eigblk != NULL) {
        if (ldisk == spcs->maxrec)
          n = (long) spcs->maxdm;
        len = n * n;
        lret = (long) fwrite(pmblk->eigblk, sizeof(double), (size_t) len,
                             spcs->scratch);
      }
      if (ldisk == spcs->maxrec && lret == len) {
        /*  fill in gaps with junk FIRST time */
        n = (long) spcs->maxdm - (long) pmblk->nsizblk;
        len = (long) ndel;
        if (n > 0) {
          len *= n;
          lret =
              (long) fwrite(pvec, sizeof(double), (size_t) len, spcs->scratch);
        }
        spcs->maxrec += spcs->lsizb;
      }
    }
    if (lret != len) {
  ...
  if (ipos < 0) {
    spcs->orgpos = spcs->mempos;
  } else {                      /* read from disk */
    ldisk = (long) (ipos - spcs->nbsav);
    if (ldisk != 0)
      ldisk *= spcs->lsizb;
    fseek(spcs->scratch, ldisk, SEEK_SET);
    pdblk = &blk[ipos];
    ...
    lret = 0;
    if (pvec != NULL) {
      lret = (long) fread(pvec, sizeof(double), (size_t) len, spcs->scratch);
