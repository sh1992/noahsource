...
/* Static variables moved to spcat_state */
const double zero = 0.;

double ctrans(spcs_t *spcs, int i1, int i2, int ii, short *qn, int nqn, 
                      /*@null@*/ /*@out@*/ short *swk);

void getzitot(spcs_t *spcs, double *zfac, int ltot, int ii, const int *lscomv, 
...
{ /* get reduced matrix elements of nuclear spins */
  ...
  for (pitot = spcs->ithead; pitot != NULL; pitot = pitot->next) {
...
void setzitot(spcs_t *spcs, int lv1, int lv2, int lvt, int ii, int neqi)
  ...
  for (itdif = 0; itdif <= lvt; ++itdif){
    for(itot = itdif; itot < itmax; ++itot){
      ...
      if (sii2 > 0) 
        lscomv[nm1 + sii2] = lv2;
      for (k = 0; k < nm1; ++k) {
        i = spcs->itpair[k + k]; j = spcs->itpair[k + k + 1];
        if (lscomv[i] == 0) {
          lscomv[k] = lscomv[j];
        } else if (lscomv[j] == 0) {
          lscomv[k] = lscomv[i];
	  ...
	  if (k == nm2) {
	    ...
	    tensor(spcs, &val, iscomv, jscomv, lscomv, spcs->itpair, nm1, -1); 
	    vec[i] += val;
	  }
	}
      }
...
const int ii, neqi;
{
  ...
  pret->next = spcs->itmix_head; spcs->itmix_head = pret; pret->ii = ii;
  pret->neqi = neqi;
  iend = neqi * ii; ibgn = iend & 1; 
  nq = iend - ibgn + 2; ntot = nq >> 1; pret->nitot = ntot; 
  pret->mix = (EITMIX *)mallocq((size_t) ntot * sizeof(EITMIX));
  if (spcs->fc_neqi != neqi) {
    spcs->zrfc[0] = 1.; spcs->zifc[0] = 0.;
    sum = 2. * acos(-1.) / neqi;
    spcs->zrfc[0] = 1.; spcs->zifc[0] = 0.;
    for (i = 1; i < neqi; ++i) {
      spcs->zrfc[i] = cos(i * sum); spcs->zifc[i] = sin(i * sum);
    }
    spcs->fc_neqi = neqi;
  } 
  nm1 = neqi - 1; nm2 = nm1 - 1; nl = (size_t) nm1 * sizeof(short); 
  spcs->itpair = &spcs->itpairs[nm2 * nm2 + nm2];
  ...
  do {
    while(k < nm2) {
      ++k;
      ia = spcs->itpair[k + k]; ib = spcs->itpair[k + k + 1];
      ia = ix[ia]; ib = ix[ib];
      ixx[k] = (short) (ia + ib);
      ia -= ib;
      if (ia < 0) 
  ...
  pitmix = spcs->itmix_head->mix; pitmix->neven = 0; pitmix->n = 0; pitmix->qn = qn; 
  ...
  nl = (size_t)(ns + ns); spcs->itmix_head->nditot = nl; 
  qn = NULL; qn0 = NULL; ixx = NULL; ix = NULL; 
  spcs->itmix_head->eigvecv = (double *)mallocq((size_t)neigv * sizeof(double));
  eigvec = spcs->itmix_head->eigvecv; pitmix = spcs->itmix_head->mix;
  *eigvec = 1.; pitmix->eigvec = eigvec; 
  ...
  for (ktot = 0; ktot < nq; ++ktot) { 
    ...
    for (i = 0; i < nitot; ++i) {
      for (k = 0; k <= i; ++k) { 
        val = ctrans(spcs, i, k, ii, qn, neqi, swk); 
        eigvec[i + k * nitot] = val;
        if (k == i) continue;
        val -= ctrans(spcs, k, i, ii, qn, neqi, swk);
        if (fabs(val) > 1.e-10) {
    ...
    for (k = 0; k < nitot; ++k) {
      ...
      if (i == ioff) continue;
      qn0 = pitmix->qn; cfac = 1. / spcs->zifc[ia];
      eiga = &pitmix->eigvec[i * ioff]; 
      eigb = &eigvec[k * nitot]; sumsq = 0.;
      for (j = 0; j < nitot; ++j) {
        sum = 0.;
        for (i = 0; i < ioff; ++i) {
          val = ctrans(spcs, i, j + ioff, ii, qn0, neqi, swk); 
          sum += val * eiga[i];
          val += ctrans(spcs, j + ioff, i, ii, qn0, neqi, swk);
          if (fabs(val) > 1.e-10) {
...
double ctrans(spcs, i1, i2, ii, qn, nqsym, swk)
     spcs_t *spcs;
     int i1, i2, ii, nqsym;
     short *qn, *swk;
{
  ...
  if (nqsym == 3) {
    ...
    do { /* loop over m */
      ...
      while (mt >= imin && mt <= ii) { 
        for (kk = 0; kk < nm1; ++kk) {
          ia = spcs->itpair[kk + kk]; ib = spcs->itpair[kk + kk + 1];
